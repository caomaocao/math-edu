...
const 行数 = 镇图.length;
const 列数 = 镇图[0].length;
const 路格们 = [];
for (let 行 = 0; 行 < 行数; 行++) {
  for (let 列 = 0; 列 < 列数; 列++) {
    if (镇图[行][列] === 'x') 路格们.push({ 行, 列 });
  }
}
...
const 站点 = [
  { 名: '游乐园', 图: '🎡', 行: 0, 列: 2, 英: 'amusement park' },
  { 名: '猫猫家', 图: '🐱', 行: 1, 列: 6, 英: "kitten's house", 昵: '小猫', 英昵: 'kitten' },
  { 名: '面包店', 图: '🍞', 行: 1, 列: 0, 英: 'bakery' },
  { 名: '博物馆', 图: '🏛️', 行: 5, 列: 4, 英: 'museum' },
  { 名: '兔子家', 图: '🐰', 行: 4, 列: 0, 英: "bunny's house", 昵: '小兔', 英昵: 'bunny' },
];
/** 这一站的昵称（没单独起昵称就用站名本身） */
...
export function 创建(面板, 工具) {
  ...
  const 点 = 做进度点(面板.querySelector('#小镇进度'), 3);

  // 铺格子
  const 格元们 = {};
  for (let 行 = 0; 行 < 行数; 行++) {
    for (let 列 = 0; 列 < 列数; 列++) {
      const 格 = 元('div', 镇图[行][列] === 'x' ? '镇格 是路' : '镇格 是草');
      镇网.appendChild(格);
      格元们[`${行},${列}`] = 格;
    }
  }
  ...
  // 站牌：贴纸图和 emoji 同占 44px 的字号盒，站的位置一点不动。
  // 「游乐园」「猫猫家」按归一表折到摩天轮和小猫那两张图上
  //（跟学校地图共用一只摩天轮、跟别的关共用同一只小猫）。
  for (const { 名, 图, 行, 列 } of 站点) {
    const 牌 = 元('span', '站牌');
    牌.appendChild(画实体(名, 图, { 类名: '站实体图' }));
    牌.title = 名;
    格元们[`${行},${列}`].appendChild(牌);
  }
  ...
  async function 进入() {
    ...
    const 今日订单 = [站点[1], 站点[0], 站点[3]]; // 猫猫家 → 游乐园 → 博物馆
    for (let 单 = 0; 单 < 今日订单.length; 单++) {
      ...
      while (还在() && !(车在.行 === 目的地.行 && 车在.列 === 目的地.列)) {
        ...
        音效.点一下();
        for (const 步 of 经过) { 摆车(步); await 歇(470); }
        车在 = 终点;
