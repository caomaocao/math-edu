...
// 这一关屏幕上出现的实体（规范名）。九只动物是提示实体（孩子答的是第几行第几列，不必说出名字）；木墩是 CSS 画的。
...
export function 创建(面板, 工具) {
  ...
  阵容.forEach(([图, 名, 英], i) => {
    const 桩 = 元('div', '树桩');
    // 木墩用 CSS 画（不用 🪵 emoji）：动物和墩子叠成一个整体，
    // 一格就是一个单元，3×3 看上去才是 3 行，不会被数成 6 行。
    // 墩上那只走实体图（小狐狸/小兔子…归一到动物地图同一张，两关是同一只）；
    // 缺图时还回文本节点，.桩上动物 的字号盒原样，位置一点不挪。
    const 动 = 元('span', '桩上动物');
