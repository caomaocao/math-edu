 ...
 *   锅样     { 实体, 兜底 } —— 给了就把那口空锅换成贴纸（素材升级10，仅分饭站用）；
 *            不给就保持 CSS 皮：蛋糕派对靠此维持大白盘、Boss 发饭关维持棕锅（都不换）。
 * 挨个亮(念) —— 报数错 2 的提示（票面：把已吃上饭的小朋友挨个点亮带数）：
 *   按队伍顺序点亮每个吃上饭的，亮一个 await 念(第几个)。
 */
export function 摆发饭台({ 挂点, 食, 账, 排队数, 走查 = () => true, 空了 = () => {}, 锅样 }) {
  挂点.textContent = '';
  ...
  锅.className = '饭锅';
  if (锅样) {
    // 空锅贴纸垫在底、锅肚（还没发的份）盖在锅口上——盛的饭是另一层，别画进锅里。
    锅.classList.add('有锅图');
    锅.appendChild(造贴(锅样.实体, 锅样.兜底, '锅图'));
  }
