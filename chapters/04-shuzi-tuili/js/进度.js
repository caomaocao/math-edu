// 进度 —— 第4讲在 localStorage 里那一份带版本号的 JSON。
// 读到版本对不上的旧数据一律当没有；存不下（无痕模式、存满）绝不抛错。
//
// 每讲一份自己的进度模块、自己的键（第2讲 cube-fold:进度:v*、第3讲 fangwei:进度:v1），
// 不共用——共享的那份 /shared/js/进度.js 的键焊死在 fangwei 上，是第3讲的。这一讲用
// 自己的键 shuzi:进度:v1，形状与合并语义跟第3讲同源（合并 有本讲自己的 node 测试，票 08）。
//
// 云端同步三处挂钩（同第3讲，见 /shared/js/云同步.js 头的三条硬约束）：
...
/**
 * 合并(本地, 云端) —— 云同步逐键调度调的纯函数。语义见第3讲：整包重置戳先行、
