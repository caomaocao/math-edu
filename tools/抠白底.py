...
# 分档出图（素材升级 票 12）：256 的老规矩按"显示 ≤64px 的 4 倍"算，对小贴纸仍然成立；
# 但大道具在关里显示 90~220 CSS px，×舞台系数×retina 要 500~600 设备像素，256 源图必糊
# （八大罗盘盘面在 16″ MBP 上糊掉就是这么来的）。这里按素材名给大道具开大档，
# 档位 ≈ 显示 CSS px × 1.3(舞台系数) × 2(retina) 向上取整到 512/768。
大图档 = {
    "罗盘盘面": 768,   # 八大罗盘显示 220 + 小罗盘 92 共用这张
    "饭锅": 512, "篝火": 512, "花坛": 512, "山洞": 512, "树桩": 512,
    "小火车": 512, "火车站": 512,
    "绿帐篷": 512, "红帐篷": 512, "蓝帐篷": 512, "棕帐篷": 512,
}
...
def 抠一张(源: Path, 目标: Path) -> None:
    ...
    # 补成带边距的正方形画布，再缩到出图尺寸（大道具按 大图档 出大图，其余 256）
    尺寸 = 大图档.get(目标.stem, 出尺寸)
    边 = int(max(图.size) * (1 + 边距比 * 2))
    布 = Image.new("RGBA", (边, 边), (0, 0, 0, 0))
    布.paste(图, ((边 - 图.width) // 2, (边 - 图.height) // 2))
    布 = 布.resize((尺寸, 尺寸), Image.LANCZOS)
